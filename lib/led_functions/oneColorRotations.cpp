...
void Pixels::oneColorRotation(uint8_t num_angles, float width_angle, int direction, float fadetime)
{
    ...
    for (uint8_t i_angle = 0; i_angle < num_angles; i_angle++)
    {
        ...
        prev_angle_start[i_angle] = angle_start[i_angle];
    ...
    for (uint16_t i_pixel = 0; i_pixel < totalPixels; i_pixel++)
    {
        ...
        for (uint8_t i_all = 0; i_all <= num_angles * 2; i_all++)
        {
